 ...
 *       这一步完全不调模型。</li>
 *   <li>关键词没命中，<b>但这句含指代词</b>（"那这个呢""它多少钱"）—— 只要粘性还在，
 *       就直接沿用上一轮的 Agent，不调模型。指代词脱离上文无法理解，
 *       这类句子几乎必然是追问，重新分类反而是白跑一次模型。</li>
 *   <li>其余情况（关键词没命中、又不含指代词）—— 走第 ④⑤ 步叫模型，连同上文一起判。</li>
 * </ul>
 *
 * <p><b>为什么判据里没有"字数"</b>：这里修过两次，两次都是"判据在猜"——
 * 先是"关键词没命中就用粘性"，后是"短的就算追问"。后者栽在中文上：
 * 中文一句话信息密度高，"挖掘机驾驶室有什么要求吗"正好 12 个字，
 * 是一句完整的问题，却被判成追问、粘在了上一轮的 Agent 上。
 * 现在只留"含指代词"这一条——它是**语义**判据，不是形态判据。
 */
@Slf4j
@Component
...
public class MsgRouter {
    ...
    private RoutingDecision doRoute(RoutingRequest request, long start) {
        ...
        // 关键词没命中：**只有含指代词时**才用粘性兜住，避免白跑一次模型。
        //
        // ⚠️ 这里修过两次，两次都是同一个毛病：**判据在猜**。
        //
        // 第一版写的是"关键词没命中就用粘性"。但"关键词没命中"不等于"是追问"——
        // "挖掘机操纵杆和其他零件的距离应该控制在多少"这类**完整的新问题**也会落空
        // （领域名词"挖掘机"刻意没放进高权重词表），于是被一直粘在上一轮的 Agent 上。
        //
        // 第二版补了个"像不像追问"的判据，其中一条是**字数**（≤12 就算追问）。
        // 这条同样是猜——中文一句话信息密度高，"挖掘机驾驶室有什么要求吗"正好 12 个字，
        // 是一句完整的问题，却被判成追问、粘在了上一轮的 RentalAgent 上。
        //
        // 现在只留一条判据：**含指代词**。"这个/那台/它"脱离上文根本无法理解，
        // 所以含指代词几乎必然是追问；反过来，不含指代词的短句（"多少钱""还有吗"）
        // 字面意思已经完整，交给第 ④⑤ 步连同上文一起判，比猜准得多。
        // 代价是这类短句要多跑一次本地小模型（免费），换来的是路由可预测。
